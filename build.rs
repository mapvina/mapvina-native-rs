//! File for defining how we download and link against `MapVina Native`.
//! Set `MLN_CORE_LIBRARY_PATH` and `MLN_CORE_LIBRARY_HEADERS_PATH` environment variables to use a local version of mapvina
//!
...
fn download_static(out_dir: &Path, revision: &str) -> (PathBuf, PathBuf) {
    ...
    let target = if cfg!(all(target_os = "linux", target_arch = "aarch64")) {
        ...
        panic!(
            "unsupported target: only linux and macos are currently supported by mapvina-native"
        );
    };

    let mut tasks = Vec::new();
    let lib_filename = format!("libmapvina-native-core-{target}-{graphics_api}.a");
    let library_file = out_dir.join(&lib_filename);
    if !library_file.is_file() {
        let static_url = format!("https://github.com/mapvina/mapvina-native/releases/download/{revision}/{lib_filename}");
        println!("cargo:warning=Downloading precompiled mapvina-native core library from {static_url} into {}", out_dir.display());
        tasks.push(Download::new(&static_url));
    }

    let headers_file = out_dir.join("mapvina-native-headers.tar.gz");
    if !headers_file.is_file() {
        let headers_url = format!("https://github.com/mapvina/mapvina-native/releases/download/{revision}/mapvina-native-headers.tar.gz");
        println!("cargo:warning=Downloading headers for mapvina-native core library from {headers_url} into {}", out_dir.display());
    ...
    let downloads = downloader
        .download(&tasks)
        .expect("Failed to download mapvina-native static lib")
        .into_iter();
...
fn extract_headers(headers_from: &Path, headers_to: &Path) {
    println!(
        "cargo:warning=Extracting headers for mapvina-native core library from {} into {}",
...
/// Get local directory or download mapvina-native into the `OUT_DIR`
///
/// Returns the path to the mapvina-native directory and the include directories.
fn resolve_mln_core() -> (PathBuf, Vec<PathBuf>) {
    let out_dir =
        PathBuf::from(env::var_os("OUT_DIR").expect("OUT_DIR is not set")).join("mapvina-native");
    ...
    if env::var_os("MLN_CORE_LIBRARY_HEADERS_PATH").is_some() {
        assert!(
            headers.is_file(),
            "The MLN headers at {} must be a gzip (tar.gz) file containing the headers. When building locally checkout <mapvina-native repository>/.github/workflows/core-release.yml commands how to create the header archive",
            headers.display()
        );
    } else {
        assert!(
    ...
    let base = extracted_path.join("vendor").join("mapvina-native-base");
...
fn build_bridge(lib_name: &str, include_dirs: &[PathBuf]) {
    ...
    build.compile("mapvina_rust_map_renderer_bindings");
...
fn bundle_precompiled() -> Info {
    ...
    println!(
        "cargo:warning=Using precompiled mapvina-native static library from {}",
        cpp_root.display()
    );
...
fn build_local(
    ...
    let mapvina_native_dir = clone_dir.join(name);

    // Some CI cache restores may leave an incomplete directory tree.
    // Require files that prove this is a usable mapvina-native checkout.
    let has_required_checkout_files = mapvina_native_dir.join("CMakeLists.txt").is_file()
        && mapvina_native_dir.join("include").is_dir();

    if mapvina_native_dir.exists() && !has_required_checkout_files {
        println!(
            "cargo:warning=Removing incomplete cached mapvina-native checkout at {}",
            mapvina_native_dir.display()
        );
        fs::remove_dir_all(&mapvina_native_dir)?;
    }

    if !mapvina_native_dir.exists() {
        println!("cargo:warning=Cloning mapvina-native.");
        fs::create_dir_all(&mapvina_native_dir)?;
        ...
        let git = |args: &[&str]| -> Result<(), Box<dyn std::error::Error>> {
            let status =
                Command::new("git").current_dir(&mapvina_native_dir).args(args).status()?;
            if !status.success() {
    ...
    let submodule_status = Command::new("git")
        .current_dir(mapvina_native_dir.clone())
        .args(["submodule", "update", "--init", "--recursive"])
        .status()?;
    if !submodule_status.success() {
        return Err(
            format!("Failed to initialize mapvina-native submodules: {submodule_status}").into()
        );
    }

    let mut config = cmake::Config::new(mapvina_native_dir.clone());
    config.build_target(TARGET_NAME);

    // mapvina-native's platform/darwin/darwin.cmake calls enable_language(Swift),
    ...
    println!(
        ...
        dest.join("build").join("vendor").join("mapvina-tile-spec").join("cpp").display()
    );
    // println!("cargo:warning=Building mapvina-native done.");

    // mapvina-native include directories
    let include_dirs: Vec<PathBuf> = [
        "include",
        "platform/default/include",
        "vendor/mapvina-native-base/include",
        "vendor/mapvina-native-base/deps/variant/include",
        "vendor/mapvina-native-base/deps/geometry.hpp/include",
        "vendor/mapvina-native-base/deps/geojson.hpp/include",
        "vendor/metal-cpp",
        "vendor/expected-lite/include",
    ]
    .into_iter()
    .map(|s| mapvina_native_dir.join(s))
    .collect();

    Ok(Info {
        lib_name: format!("{TARGET_NAME}{}", if amalgam_lib { "-amalgam" } else { "" }),
        include_dirs,
        cpp_root: mapvina_native_dir,
    })
}
...
fn build_mln() {
    ...
    let info = if precompiled {
        ...
        const MAPVINA_NATIVE_DIR_NAME: &str = "mapvina-native";
        let root = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
        let clone_dir = root.join("target");

        match build_local(&clone_dir, MAPVINA_NATIVE_DIR_NAME, amalgam_lib, &target_os) {
            Err(e) => {
                if clone_dir.join(MAPVINA_NATIVE_DIR_NAME).exists() {
                    // let _ = fs::remove_dir_all(clone_dir.join(MAPVINA_NATIVE_DIR_NAME));
                }
                panic!("Failed to build mapvina native: {e}")
            }
            Ok(info) => info,
        }
    };
    ...
    if !amalgam_lib {
        ...
        println!("cargo:rustc-link-lib=mlt-cpp"); // provided with mapvina-native
        if is_apple {
