...
# Show current mapvina-native dependency information
mapvina-native-info: (assert-cmd "curl") (assert-cmd "jq")
    #!/usr/bin/env bash
    set -euo pipefail
...
# Update mapvina-native dependency to latest core release
update-mapvina-native: (assert-cmd "curl") (assert-cmd "jq")
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    # Hit the GitHub releases API for mapvina-native and pull the latest
    ...
    if [[ -z "$LATEST_MLN_CORE_RELEASE_SHA" || "$LATEST_MLN_CORE_RELEASE_SHA" == "null" ]]; then
        echo "ERROR: no Mapvina Native Core release found"
    ...
    if [[ "$MLN_CORE_RELEASE_SHA" == "$LATEST_MLN_CORE_RELEASE_SHA" ]]; then
        ...
        echo "Updating Mapvina Native Core from $MLN_CORE_RELEASE_SHA to $LATEST_MLN_CORE_RELEASE_SHA"
